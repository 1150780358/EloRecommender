...
用法:ELO_SEED=777 python src/v5_fusion.py
"""
import json
import os
...
def load_bases():
    ...
    """
    bases = {}
    for d, pre in [(BASE_DIR, ""), (BASE_DIR + "_ext", "x_")]:
...
def derive(name, tr, va, bases, ybin, p_src="clf"):
...
def evaluate(feats, model, bases, y, ybin, folds, cond=None, p_src="clf"):
    ...
    for tr, va in folds:
        ...
        for f in feats:
            if f in DERIVED:
                a, b, c = derive(f, tr, va, bases, ybin, p_src)
            else:
                o, t = bases[f]
                a, b, c = o[tr], o[va], t
            cols_tr.append(a)
...
def main():
    ...
    out = "outputs/submission_v5_fusion.csv"
