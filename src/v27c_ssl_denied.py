# -*- coding: utf-8 -*-
"""v27-C:含被拒交易的 SSL 预训练 + 全量特征微调(docs/202608/04-v27_自监督预训练clf.md)。

churn 机制:卡冻结 → 连续被拒 → 流失;v14 已证被拒序列携带独有信息,但 v17 张量把
authorized=0 的行全部排除了。重建:hist 全量行,authorized_flag 作第 8 数值通道,
近 128 笔(含被拒)→ masked 预训练(3 epoch)→ 全量工程特征微调。判据链同 v27-B。
用法:ELO_SEED=777 python src/v27c_ssl_denied.py
"""
import os
import sys
import time
...
from v27b_ssl_full import build_full_static

rmse = lambda a, b: float(np.sqrt(mean_squared_error(a, b)))
t0 = time.time()
L = 128
DN_CACHE = "outputs/tx_tensor_dn.npz"
...
def build_tx_dn():
    ...
    feats = np.stack([
        ...
        np.zeros(len(tx)),                                # is_pad=0
        (tx["authorized_flag"] == 0).to_numpy(np.float64),  # is_denied
...
def main():
    ...
    head_dn = nn.Linear(D, 1).to(dev)                     # 新:预测被遮位是否被拒
    ...
    np.savez("outputs/base_nn_clf/ssl_dn_clf.npz", oof=oof, pred=pred)
    ...
    # 三方 rank 混合:lgb + v27B + v27C
    zb = np.load("outputs/base_nn_clf/ssl_full_clf.npz")
    ...
    for w1 in np.arange(0, 0.45, 0.05):
        for w2 in np.arange(0, 0.45 - w1 + 1e-9, 0.05):
            w0 = 1 - w1 - w2
            o = w0 * cands["lgb"][0] + w1 * cands["sslB"][0] + w2 * cands["sslD"][0]
            a = roc_auc_score(yb, o)
            if a > best_auc:
                best_auc, best_ws = a, (w0, w1, w2)
    w0, w1, w2 = best_ws
    ...
    r0, _, _ = vf.evaluate(allf, "bayes", bases, y_s, yb, folds, p_src="f_clf", clean_src="f_clean")
    results = {}
    bases["p_ssl"] = (oof, pred)
    r1, _, _ = vf.evaluate(allf + ["p_ssl"], "bayes", bases, y_s, yb, folds, p_src="f_clf", clean_src="f_clean")
    results["追加 p_sslD 成员"] = r0 - r1
    del bases["p_ssl"]
    bases["f_clf"] = (bl_oof, bl_pred)
    r2, _, pt = vf.evaluate(allf, "bayes", bases, y_s, yb, folds, p_src="f_clf", clean_src="f_clean")
    results["f_clf 替换为三方混合"] = r0 - r2
    for tag, d in results.items():
        log(f"  {tag}: Δ={d:+.5f}")
    best_d = max(results.values())
    log(f"判据[v27-C SSL+denied]:基线={r0:.5f} 最优 Δ={best_d:+.5f} "
        f"{'✅ 通过' if best_d > 0.0005 else '❌ 不足'}")
    if best_d > 0.0005:
        sub = pd.read_csv(os.path.join(ep.CONFIG["DATA_DIR"], "sample_submission.csv"))
        sub["target"] = pt
        sub.to_csv("outputs/submission_v27_ssl.csv", index=False)
        log("已保存 outputs/submission_v27_ssl.csv")
    else:
        sys.exit(3)
