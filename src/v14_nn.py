...
判据在融合层(v5_fusion 新 F31 vs 旧 F31 3.62162)。
用法:ELO_SEED=777 python src/v14_nn.py data_ext
     ELO_SEED=777 python src/v14_nn.py train <gru|gru_x|trf> <dev_id> [seeds,逗号分隔]
"""
import os
import sys
import time
...
def train(arch, dev_id, seeds):
    ...
    log(f"[{arch}] train {xs_tr.shape} dev={dev} seeds={seeds}")
