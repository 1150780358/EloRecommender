# -*- coding: utf-8 -*-
"""v17:交易级序列模型(docs/202608/03-v17_交易级序列.md)。

每卡最近 128 笔交易(hist auth=1 + new,时间升序,pad 前置)作为 token 序列,
数值 7 + 类别 emb(品类/行业),GRU 直接回归 target。静态分支复用 v13 的瘦 5 列。
用法:ELO_SEED=777 python src/v17_txseq.py data
     ELO_SEED=777 python src/v17_txseq.py train <dev_id> <seeds逗号> [out_suffix]
     (各卡跑完后)ELO_SEED=777 python src/v17_txseq.py merge
"""
import os
import sys
import time
...
TX_CACHE = "outputs/tx_tensor.npz"
...
def build_tx():
    ...
    tx["mcat"] = (mcat + 2).astype(np.int16)   # 0=pad,1=nan
    ...
    np.savez_compressed(TX_CACHE,
                        num_tr=num[is_tr], num_te=num[~is_tr],
                        cat_tr=cats[is_tr], cat_te=cats[~is_tr],
                        st_tr=zs["st_tr"], st_te=zs["st_te"],
...
def train(dev_id, seeds, suffix=""):
    ...
    for sd in seeds:
        ...
        np.savez(os.path.join(PARTS_DIR, f"gru_t_s{sd}.npz"), oof=oof, pred=pred)
...
def merge():
    ...
    log(f"[gru_t] merge {len(oofs)} seed 平均 OOF={rmse(y, np.mean(oofs, 0)):.5f}"
        f" -> {OUT_DIR}/gru_t.npz")
